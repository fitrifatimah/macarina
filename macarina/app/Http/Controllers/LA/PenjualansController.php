...
class PenjualansController extends Controller
{
	...
	public $view_col = 'level';
	public $listing_cols = ['id', 'macarina', 'user', 'level', 'tanggal', 'jumlah'];
	...
	{
		$values = DB::table('penjualans')->select($this->listing_cols)->whereNull('deleted_at')->where('user','=',Auth::user()->name) ;
		$out = Datatables::of($values)->make();
		$data = $out->getData();
