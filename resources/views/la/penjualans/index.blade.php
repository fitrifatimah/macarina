	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<input type="hidden" name="tanggal" value="{{ Carbon\Carbon::now()->format('d/m/Y') }}">
...
$(function () {
	$("#example1").DataTable({
		processing: true,
        serverSide: true,
        ajax: "{{ (Auth::user()->name == 'graha') ? url(config('laraadmin.adminRoute') . '/penjualan_dt_ajax') : url(config('laraadmin.adminRoute') . '/penjualan_dt_ajax2') }}",
		language: {
			lengthMenu: "_MENU_",
			search: "_INPUT_",
